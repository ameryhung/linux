 ...
 * A pointer to the TLD will be return. It also caches the key in a task local
 * storage map, tld_key_map, whose member must be defined by the developer.
 *
 * struct tld_keys {
 *     tld_key_t prio;
 *     tld_key_t in_cs;
...
#ifndef TLD_DATA_CNT
#define TLD_DATA_CNT (__PAGE_SIZE / sizeof(struct tld_metadata) - 1)
#endif
...
__attribute__((unused))
static int __tld_fetch_key(struct tld_object *tld_obj, const char *name, int from)
{
