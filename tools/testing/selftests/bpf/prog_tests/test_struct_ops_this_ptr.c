...
static void test_struct_ops_this_ptr_in_timer_common(int timer_nsec, int expected_data)
{
	...
	map_fd = bpf_map__fd(skel->maps.testmod_this_ptr);
	...
	/* Detach struct_ops and free the map */
...
static void test_struct_ops_this_ptr_in_timer_after_free(void)
{
	/*
...
void serial_test_struct_ops_this_ptr(void)
{
	...
	if (test__start_subtest("struct_ops_this_ptr_in_timer_after_free"))
		test_struct_ops_this_ptr_in_timer_after_free();
}
