...
static void reset_tld(void)
{
	if (TLD_READ_ONCE(tld_metadata_p)) {
		...
		memset(&tld_metadata_p->metadata[1], 0,
		       (TLD_DATA_CNT - 1) * sizeof(struct tld_metadata));
	}
}
...
#define TEST_BASIC_THREAD_NUM TLD_DATA_CNT
...
static void test_task_local_data_basic(void)
{
	...
	/*
	 * Create a TLD just big enough to exceed a page. TLDs already created are
	 * int value0, int value1, and struct test_struct value2.
	 */
...
#define TEST_RACE_THREAD_NUM (TLD_DATA_CNT - 3)
