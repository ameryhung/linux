 ...
 * OPTIONS
 *
 *   Define the options before including the header
 *
 ...
 *   as not all memory allocator can use the exact amount of memory requested to fullfill
...
#ifndef TLD_DATA_CNT
#define TLD_DATA_CNT (TLD_PAGE_SIZE / sizeof(struct tld_metadata) - 1)
#endif
...
struct u_tld_data {
	__u64 start; /* offset of data in a page */
	char *data[];
};
...
__thread void *tld_data_buf_p __attribute__((weak));
 ...
 * @key: The variable name of the key
 *
 ...
 *
 * A per-thread buffer of size TLD_DYN_DATA_SIZE will be allocated to accommodate TLDs
 ...
 * for this thread, or the key is invalid.
 *
...
static void *tld_get_data(int map_fd, tld_key_t key)
{
	...
	return (char *)tld_data_p->data + key.off;
}
