...
{
	struct list_head *list = vsock_bound_sockets_in_table(bind_table, addr);
	struct vsock_sock *vsk;

	list_for_each_entry(vsk, list, bound_table) {
...
vsock_find_bound_dgram_socket(struct sockaddr_vm *addr)
{
	...
	sk = vsock_find_bound_socket_common(addr, vsock_dgram_bind_table);
	if (sk)
		sock_hold(sk);
...
{
	return vsock_find_bound_socket_common(addr, vsock_bind_table);
}
...
static void vsock_insert_dgram_unbound(struct vsock_sock *vsk)
{
	spin_lock_bh(&vsock_dgram_table_lock);
...
vsock_connectible_lookup_transport(unsigned int cid, u8 flags)
{
...
vsock_dgram_lookup_transport(unsigned int cid, u8 flags)
{
	const struct vsock_transport *transport;
			     ...
			     struct list_head *bind_table)
{
	...
	if (addr->svm_port == VMADDR_PORT_ANY) {
		...
		for (i = 0; i < MAX_PORT_RETRIES; i++) {
			...
			if (!vsock_find_bound_socket_common(&new_addr, bind_table)) {
				found = true;
				break;
			}
		}
	 ...
	 */
	__vsock_remove_bound(vsk);
	__vsock_insert_bound(vsock_bound_sockets_in_table(bind_table, &vsk->local_addr),
			     vsk);
...
static int __vsock_bind_connectible(struct vsock_sock *vsk,
				    struct sockaddr_vm *addr)
{
	return vsock_bind_common(vsk, addr, vsock_bind_table);
}

static int __vsock_bind_dgram(struct vsock_sock *vsk,
			      struct sockaddr_vm *addr)
{
	/* Virtio simply uses vsock_dgram_bind_table to record bound sockets
	 * instead of implementing custom logic in dgram_bind() like VMCI.
	 */
			...
			int protocol, int kern)
{
	...
	if (sock->type == SOCK_DGRAM) {
		...
		vsock_insert_dgram_unbound(vsk);
	} else {
